...
void MMM_CreateButton(t_button_info* buttonArray,int buttonsNum,BUTTON_Handle* handleArray)
{
    ...
    int Flags = BUTTON_CF_SHOW;
    
    for(i=0; i<buttonsNum; i++){
		if(buttonArray[i].IsVisibleFunc && buttonArray[i].IsVisibleFunc())
		{
			Flags = BUTTON_CF_HIDE;
		}
		else
		{
			Flags = BUTTON_CF_SHOW;
		}
...
void MMM_DrawLableItem(t_lable_info* lable,int isXor)
{
    char lable_text[TEXT_MAX_LENTH];
    const char* lablePtr;
    GUI_FONT* oldFont = NULL;
    if(lable->font)
        oldFont = GUI_SetFont(lable->font);
    else
        oldFont = NULL;
    if(lable->getLable)
    {
        lable_text[0] =0;
        lable->getLable(lable_text); 
        lablePtr = (const char*)lable_text;
    }
    else 
    {
        lablePtr = (const char*)lable->text;
    }
    if(lablePtr)
    {
        GUI_DispStringInRect(lablePtr, &lable->rect, GUI_TA_HCENTER | GUI_TA_VCENTER);
    }
    if(oldFont)
       GUI_SetFont(oldFont); 
