...
EDIT_PROPS EDIT__DefaultProps = {
  ...
  EDIT_BORDER_DEFAULT,
  EDIT_FONT_DEFAULT,
  {EDIT_TEXTCOLOR0_DEFAULT,EDIT_TEXTCOLOR1_DEFAULT},
  {EDIT_BKCOLOR0_DEFAULT,EDIT_BKCOLOR1_DEFAULT}
};
...
*/
static void _Paint(EDIT_Obj* pObj, EDIT_Handle hObj) {
  GUI_RECT rFillRect, rInside, r, rText, rInvert = {0};
  const char GUI_UNI_PTR * pText = NULL;
  int IsEnabled, CursorWidth = 1;
  ...
  WM_ITERATE_START(NULL) {
    ...
    if (pObj->Widget.State & WIDGET_STATE_FOCUS) {
		...
		if(!Timer1){
			  Timer1 = GUI_TIMER_Create((GUI_TIMER_CALLBACK*)ShowCurrsor, 2, 0, 0);	//houhh 20061018...
