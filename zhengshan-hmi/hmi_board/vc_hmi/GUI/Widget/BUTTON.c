...
static void _Paint(BUTTON_Obj* pObj, BUTTON_Handle hObj) {
  const char* s = NULL;
  unsigned int Index;
  int State, PressedState, ColorIndex,unDrawBorder,invert;
  GUI_RECT rClient, rInside;
  State = pObj->Widget.State;
  PressedState = (State & BUTTON_STATE_PRESSED) ? 1 : 0;
  unDrawBorder = (State & BUTTON_UNDRAW_BORDER) ? 1 : 0;
  invert = (State & BUTTON_INVERT_DRAW) ? 1 : 0;
...
/* Draw the actual button (background and text) */  
  ...
  if(invert & PressedState)
  {
      GUI_InvertRect(rClient.x0,rClient.y0,rClient.x1,rClient.y1);
  }
}
