...
void _MainMenu(void) {
    ...
    for(i=0; i<GROUPS_NUM; i++){
        ...
        BUTTON_SetState(ahGsButton[i],BUTTON_INVERT_DRAW);
        BUTTON_SetState(ahVvButton[i],BUTTON_INVERT_DRAW);
    ...
    switch (r) {
        ...
        case    ID_MM_GROUP_STATE_4:
             if(_GroupState(r- ID_MM_GROUP_STATE_1) == WAIT_EVENT_TIMEOUT)
			 	return;
             break;
        ...
        case    ID_MM_VALVE_VIEW_4:
            if(_ValveView(r- ID_MM_VALVE_VIEW_1) == WAIT_EVENT_TIMEOUT)
				return;
            break;
        case ID_STATE_NONE:
            return;
