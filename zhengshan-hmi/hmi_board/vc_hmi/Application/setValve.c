...
static const t_button_info s_sv_button[]={
    {ID_SV_ENTER,&bmvc_enter_pressed,&bmvc_enter_unpressed,&bmvc_enter_pressed,{243,11,301,47},NULL},
};
...
int _SetValve(void )
{
    ...
    r= MMM_WaitEvent(WAITEVENT_TIME,(t_lable_info*)&s_sv_lable[0],countof(s_sv_lable),NULL);
    MMM_DeleteButton(buttonHandle,countof(s_sv_button));
        ...
        switch(r){
           ...
           case ID_SV_CONTRAL_OPEN_ADDR:
               	r = _VVEditNum("Open Addr",text,VALVE_ADDR_MAX_LENTH);
			   	if((r != 0) && (r != WAIT_EVENT_TIMEOUT))
			       s_valve_data.xCmd.dBOffsetForOpenCmd = atoi(text);
			   	else if(r == WAIT_EVENT_TIMEOUT)
			   		return r;
               break;
           case ID_SV_CONTRAL_OPEN_CMD:
               	r = _VVEditNum("Open Cmd",text,VALVE_CMD_MAX_LENTH);
				if((r != 0) && (r != WAIT_EVENT_TIMEOUT))
           ...
           case ID_SV_CONTRAL_CLOSE_ADDR:
               	r = _VVEditNum("Close Addr",text,VALVE_ADDR_MAX_LENTH);
			   	if((r != 0) && (r != WAIT_EVENT_TIMEOUT))
           ...
           case ID_SV_CONTRAL_CLOSE_CMD:
               	r = _VVEditNum("Close Cmd",text,VALVE_CMD_MAX_LENTH);
			   	if((r != 0) && (r != WAIT_EVENT_TIMEOUT))
           ...
           case ID_SV_CONTRAL_STOP_ADDR:
               	r = _VVEditNum("Stop Addr",text,VALVE_ADDR_MAX_LENTH);
				if((r != 0) && (r != WAIT_EVENT_TIMEOUT))
           ...
           case ID_SV_CONTRAL_STOP_CMD:
               	r = _VVEditNum("Stop Cmd",text,VALVE_CMD_MAX_LENTH);
			   	if((r != 0) && (r != WAIT_EVENT_TIMEOUT))
           ...
           case ID_SV_CONTRAL_POSC_CMD:
               	r = _VVEditNum("POSC Cmd",text,VALVE_CMD_MAX_LENTH);
				if((r != 0) && (r != WAIT_EVENT_TIMEOUT))
           ...
           case ID_SV_CONTRAL_POS_ADDR:
               	r = _VVEditNum("Pos Addr",text,VALVE_ADDR_MAX_LENTH);
				if((r != 0) && (r != WAIT_EVENT_TIMEOUT))
          ...
          case ID_SV_CONTRAL_POS_MAX_VAL:
               	r = _VVEditNum("Pos Max Val",text,VALVE_CMD_MAX_LENTH);
				if((r != 0) && (r != WAIT_EVENT_TIMEOUT))
          ...
          case ID_SV_VIEW_OPEN_ADDR:
           ...
           case ID_SV_VIEW_LR_ADDR:
               	r = _VVEditNum("LR Addr",text,VALVE_ADDR_MAX_LENTH);
				if((r != 0) && (r != WAIT_EVENT_TIMEOUT))
           ...
           case ID_SV_VIEW_POS_MAX_VAL:
                ...
                break;
           case ID_SV_SET_NAME:
                r = _VVEdit("Set Name",text,VALVE_NAME_MAX_LENTH);
				if((r != 0) && (r != WAIT_EVENT_TIMEOUT))
