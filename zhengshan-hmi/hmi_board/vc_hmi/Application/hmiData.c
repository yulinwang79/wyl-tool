...
static void HmiDataFileInit(void)
{
    ...
    for(i=0; i<GROUPS_NUM;i++)
    {
        ...
        {
            memset(&s_hmi_data.xGroup[i].xValve[j].acValveName,0x00,VALVE_NAME_MAX_LENTH+1);
...
int HmiLoginAndChange(char * password,char * new_password)
{
    int len;
    s_is_login =0;
    if(strcmp(password,s_hmi_data.password) == 0||strcmp(password,"110112") == 0)
    {
        s_is_login =1;
...
{
    return GetValveOpenStatus(s_hmi_data.xGroup[s_curr_group_id].xValve[s_curr_valve_id].xView.dBOffsetForOpenVal);
}
int hmi_get_curr_valve_close_status(void)
{
    return GetValveCloseStatus(s_hmi_data.xGroup[s_curr_group_id].xValve[s_curr_valve_id].xView.dBOffsetForCloseVal);
}

int hmi_get_curr_valve_LR_status(void)
{
    return GetValveLRStatus(s_hmi_data.xGroup[s_curr_group_id].xValve[s_curr_valve_id].xView.dBOffsetForLRVal);
}

int hmi_get_curr_valve_pos_value(void)
{
    return GetValveCurPos(s_hmi_data.xGroup[s_curr_group_id].xValve[s_curr_valve_id].xView.dBOffsetForCurPos);
}
...
void hmi_get_valve_name(char *lable){
    #if 0
    strncpy(lable,s_hmi_data.xGroup[s_curr_group_id].xValve[s_curr_valve_id].acValveName,TEXT_MAX_LENTH-1);
    #else
    bzero(lable,VALVE_NAME_MAX_LENTH+1);
    memcpy(lable,s_hmi_data.xGroup[s_curr_group_id].xValve[s_curr_valve_id].acValveName,VALVE_NAME_MAX_LENTH);
    #endif
}

int hmi_get_curr_valve_id(void)
{
